...
Release: **v2.0.3** · Image: **`ghcr.io/mckenna654/nuvio2fusion:2.0.3`**
...
1. Download [`unraid-template.xml`](https://github.com/mckenna654/nuvio2fusion/releases/download/v2.0.3/unraid-template.xml) from the release.
...
The template pins version `2.0.3` for a repeatable install. Its WebUI, icon, support link and port mapping are included. Unraid's official guide covers [bridge networking, port mappings and container management](https://docs.unraid.net/unraid-os/using-unraid-to/run-docker-containers/managing-and-customizing-containers/).
...
| --- | --- |
| Name | `Nuvio2Fusion` |
| Repository | `ghcr.io/mckenna654/nuvio2fusion:2.0.3` |
| Network Type | `Bridge` |
| Privileged | `Off` |
...
Download [`docker-compose.release.yml`](https://github.com/mckenna654/nuvio2fusion/releases/download/v2.0.3/docker-compose.release.yml). The default host binding is localhost. For a trusted LAN, set `NUVIO2FUSION_BIND_IP` to your server's LAN IP before starting, replacing the example address below:
...
- **Verify:** open `http://YOUR-UNRAID-IP:7088/api/health`; this release reports `Nuvio2Fusion`, version `2.0.3`, status `ok`. The image also includes a Docker health check.
...
- **Permission denied when pulling:** the package is public; use the exact `ghcr.io/mckenna654/nuvio2fusion:2.0.3` image name and check GitHub/registry connectivity.
